...
currentTrackDiv = function(path){
	...
	if(engineState == ENGINE_STATE_PAUSE || engineState == ENGINE_STATE_PLAY){
		...
		div = div.replace('###rating-guest###', rating );
		div = div.replace('###rating-dj###', rating );
...
// TODO: check compatibility with Win-Amarok
	...
	div = loadHeaderFile() + div + loadFooterFile();
	response.append(div);
	return response;
}
...
playlistDiv = function(path){
	...
	if ( nbTracks == 0 ) {
		...
		for(trackidx=0; trackidx<nbTracks; trackidx=trackidx+1){
			...
			if ( hasAccess("/api/cmd/playByIndex") ) {
				tracks += '<a href="#" data-amarok-track-id="'+trackidx+'">';
			}
			...
			tracks += ' alt="" /><p>'+t.artist+'</p>'+t.title;

			if ( hasAccess("/api/cmd/playByIndex") ) {
				tracks += '</a>';
			}
			if ( hasAccess("/api/cmd/removeByIndex") ) {
